 ...
 *
 * A list of permissions for $apiPermissionScope are at
 * <https://cms.paypal.com/mx/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_PermissionsGetPermissionsAPI>
 *
 */
...
class ConnectionsPaypalSource extends DataSource {
	...
	public $config = array('environment' => 'sandbox', 'sandboxEmailAddress' => '', 'apiUsername' => '', 'apiPassword' => '', 'apiSignature' => '', 'apiAppId' => 'APP-80W284485P519543T',
	// // Global Sandbox Application ID
	'apiPermissionScope' => array(), 'chainedPrimaryEmail' => '', 'chainedPrimaryPercentage' => 0);
	...
	public function sendRequest($apiOperation, $body) {
		...
		$body['requestEnvelope']['detailLevel'] = 'ReturnAll';
		// 0
		#ksort($query);

		$request = array('method' => 'POST', 'uri' => $this->config['endpoint'] . $apiOperation, 'header' => array('X-PAYPAL-SECURITY-USERID' => $this->config['apiUsername'], 'X-PAYPAL-SECURITY-PASSWORD' => $this->config['apiPassword'], 'X-PAYPAL-SECURITY-SIGNATURE' => $this->config['apiSignature'], 'X-PAYPAL-APPLICATION-ID' => $this->config['apiAppId'], 'X-PAYPAL-DEVICE-IPADDRESS' => $_SERVER['REMOTE_ADDR'], 'X-PAYPAL-REQUEST-DATA-FORMAT' => 'JSON', 'X-PAYPAL-RESPONSE-DATA-FORMAT' => 'JSON', ), 'body' => json_encode($body));
	...
	private function _ackHandler($result) {
		$ackCodes = array('Success' => array('okay' => true, 'log' => false), 'Failure' => array('okay' => false, 'log' => true), 'Warning' => array('okay' => false,
		// //???
		'log' => true), 'SuccessWithWarning' => array('okay' => true, 'log' => true), 'FailureWithWarning' => array('okay' => false, 'log' => true), );
